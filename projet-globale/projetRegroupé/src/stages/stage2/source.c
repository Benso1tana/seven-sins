...
static void initialiserFlamme(Flamme *f, SDL_Surface *img, int x, int y, int vitesse) {
    f->img = img;
    f->pos.x = x;
    f->pos.y = y;
    f->vitesse = vitesse;
...
static void animerFlamme(Flamme *f, int hauteurEcran, SDL_Surface *flamme1, SDL_Surface *flamme2) {
    f->pos.y += f->vitesse;
...
static void afficherFlamme(Flamme f, SDL_Surface *ecran, Background bg) {
    SDL_Rect dst = { f.pos.x - bg.camera.x, f.pos.y - bg.camera.y, f.img->w, f.img->h };
    SDL_BlitSurface(f.img, NULL, ecran, &dst);
}

static void initialiserKaelios(Kaelios *k, SDL_Surface *img) {
    k->pos.x = 500;
    k->pos.y = 600;
    k->pos.w = 100;
    k->pos.h = 150;
    ...
    for (int i = 0; i < 6; i++) {
        ...
        sprintf(pathD, "assets/images/sprites/droite stage 2/marcher%d.png", i + 1);
        sprintf(pathG, "assets/images/sprites/gauche stage 2/marcher%d.png", i + 1);
...
static void deplacerKaelios(Kaelios *k, const Uint8 *keys, Background bg) {
    if (keys[SDLK_RIGHT]) {
        k->pos.x += 5;
        k->direction = 1;
...
static void afficherKaelios(Kaelios *k, SDL_Surface *ecran, Background bg) {
...
static void verifierCollisionFlammes(Kaelios *k, Flamme *flammes, int nb) {
    for (int i = 0; i < nb; i++) {
...
static void afficherBarreVie(Kaelios *k, SDL_Surface *ecran) {
